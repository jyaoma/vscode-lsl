...
import { LSLEvent, LSLParam } from './lslTypes';
...
const writeFunction = (item: string, source: string) => {
	let relevantSource = '';
	source.split('}}{{LSL Event/Head').forEach(source => {
		if (source.includes(`event=${item}`)) {
...
const newItems: { [key: string]: LSLEvent } = alreadyDefinedConstants;
items.forEach(item => {
